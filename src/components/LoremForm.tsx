...
export default function LoremForm() {
...
I just told you! You've killed me! Fry! Quit doing the right thing, you jerk! Michelle, I don't regret this, but I both rue and lament it. Morbo can't understand his teleprompter because he forgot how you say that letter that's shaped like a man wearing a hat.`,
    ...
    const submit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (isNaN(value) || value <= 0 || value > 9) {
             setResult([<p className="result">{text[random]}</p>]);
        } else {
            let tempText: string[] = text.slice(0, value);
            let tempJSX: React.JSX.Element[] = tempText.map(function (paragraph) {
                return <p className="result">{paragraph}</p>;
            });
            setResult(tempJSX);
        }
    }
    ...
    return (
        <>
            ...
            <form className="lorem-form" onSubmit={submit}>
                <label htmlFor="amount">paragraphs:</label>
                <input type="number" name="amount" id="amount" onChange={event => setAmount(event.target.value)} placeholder="5"/>
                <button type="submit" className="btn">generate</button>
            </form>
            <article className="lorem-text">{result}</article>
        </>
    )
}